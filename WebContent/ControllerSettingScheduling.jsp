...
<body>
	...
	<div class="top1">
		<p style="margin-left:30px;color:#e7e9f4">Schedule Paper Setting > Year <%=year%> > Sem <%=new AcadYearDao().getSemester(year)%></p>
	</div>
	...
	<div class="vertical_menu_bar">
		...
		<form action="ControllerSettingScheduling.jsp">
			<input type="hidden" name="year" value="1">
		    <input class="options" style="top:260px;padding:17px 35px 17px 56px;border:none;cursor:pointer;background-color:#16181d;color:#80b5eb" type="Submit" value="Paper Setting"></br>
		</form>
		...
		<div style="width:50%;position:absolute;top:100px;left:23%">
			<table id="tb">
				...
				for(int j = 0; j < 3; j++){
					...
					if(examTask != null){
						if(examTask.getSettingOpenDate() != null) openDatePH = examTask.getSettingOpenDate();
						if(examTask.getSettingCloseDate() != null) closeDatePH = examTask.getSettingCloseDate();
					}	
				%>
				<tr>
					<form action="ControllerSettingScheduling" method="post">
